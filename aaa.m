%DVB-T 2K Transmission %The available bandwidth is 8 MHz 
...
t=0:1/Rs:Tu;
%Data generator (A)
M=Kmax+1; rand('state',0); 
a=-1+2*round(rand(M,1)).'+i*(-1+2*round(rand(M,1))).';
A=length(a); info=zeros(FS,1); info(1:(A/2)) = [ a(1:(A/2)).']; %Zero padding
...
p=1/Rs:1/Rs:T/2; g=ones(length(p),1); %pulse shape
figure(3); 
stem(p,g); 
dummy=conv(g,chips(:));
...
[H,F] = freqz(b,a,FS,Rs); 
figure(6); plot(F,20*log10(abs(H))); 
uoft = filter(b,a,u); %baseband signal (D)
figure(7); subplot(211);
plot(t(80:480),real(uoft(80:480))); 
subplot(212); plot(t(80:480),imag(uoft(80:480))); 
figure(8); subplot(211);
